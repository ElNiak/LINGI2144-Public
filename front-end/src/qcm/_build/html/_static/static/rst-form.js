...
$(function() {
    ...
    function addButtonVerify(path) {
        if(path.contains("/qcm/forstudent") || path.contains("/qcm/index")) {
            ...
            element.innerHTML = '<h2>Verify your answers<a class="headerlink" href="#checker" title="Permalink to this headline">¶</a></h2><div id="results" style="display: none;"></div><button type="submit" class="btn btn-primary" id="submit">Verify</button>';
        }
    }
...
function check() {
    ...
    $('.question').each(function(index) {
        ...
        cont = cont && all_marked($('.question').get(index));
    });

    if(!cont) {
        $('#checker').append('<div class="result"> <h4 style="text-align: center;"> Answer to all questions before </h4> </div>');
    } else {
        $('.question').each(compute_score);
        $('#checker').append('<div class="result">' + translations[language]['score'] + ' ' +
                        score + '/' +
                        $('.positive').parent().length + '</div>'); //.positive question
    }
}

function all_marked(elem) {
    if ($(this).children('.textbox').length > 0) {
        return false;
    }
    var checkebox =  $("#"+elem.id).children("div").children('.choice:checkbox:checked').length
    var radio =  $("#"+elem.id).children("div").children('.choice:radio:checked').length
    if(checkebox + radio === 0) {
        return false;
    }
    return true;
}
